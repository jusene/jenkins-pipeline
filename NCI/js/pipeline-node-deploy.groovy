        ...
        node('master') {
            ...
            node('slave-02') {
                ...
                stage('传输包到存储服务器') {
                    ...
                    sh "scp ${APPNAME}.tar.gz root@192.168.66.94:/ddhome/package/${PROJECT}/${VERSION}/FE/${BRANCH}/${datetime}"
            ...
            node('master') {
                ...
                stage("发布${BRANCH}环境"){
                    sh "python deploy.py -a ${APPNAME} -p ${PROJECT} -v ${VERSION} -d ${DIR} -f ${FIRST} -n ${NODEJS_NAME} -e ${BRANCH} -u http://192.168.66.94:88 -t ${datetime}"
                }
            }
        }   
} catch(err) {
