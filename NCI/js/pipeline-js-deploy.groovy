    ...
    try{
        if ( "${ROLLBACK}" ) {
            ...
            stage('传输包到存储服务器') {
                ...
                sh "scp ${PACKAGE_DIR}/${APPNAME}.tar.gz root@127.0.0.1:/ddhome/package/${PROJECT}/${VERSION}/FE/${BRANCH}/${datetime}"
            ...
            stage("发布${BRANCH}环境"){
                sh "python deploy.py -a ${APPNAME} -p ${PROJECT} -v ${VERSION} -d ${DIR} -e ${BRANCH} -u http://192.168.55.156:88 -t ${datetime}"
            }
        }   
    } catch(err) {
         echo "Caught: ${err}"
